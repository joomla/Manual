...
The simplest solution to achieving the effect you might expect, or want, from max, min, and step settings, is to provide a javascript function called by `onchange="myfunc(this)"` in the field xml definition. This can enforce the behaviour you want by correcting any inconsistent values. Passing 'this' as a parameter will give you access to all the field attributes. 
