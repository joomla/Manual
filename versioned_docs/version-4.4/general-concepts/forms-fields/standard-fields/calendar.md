...
- **translateformat** (optional) If set to true, the calendar will use the DATE_FORMAT_CALENDAR_DATETIME language key (if showtime is true) or DATE_FORMAT_CALENDAR_DATE (if showtime is false) to determine the format. The format attribute is ignored. If false, the format attribute is used but note that the format string must include time fields for the time to be recorded. Defaults to false.
- **showtime** (optional) When translateformat is true showtime determines whether to use the DATE_FORMAT_CALENDAR_DATETIME language key (if showtime is true) or DATE_FORMAT_CALENDAR_DATE (if showtime is false) to determine the format.
...
- **default** (optional): a default date can be specified in `Y-m-d` format, or default date and time in `Y-m-d H:M:S` format.
  - You can default to the current date and time by using default="now".
  - You can default to the current date with a time of "00:00:00" by using default="today".

You can also directly set the date and time to the current date and time by typing the word "now" into the rendered text box instead of pressing the "Today" button.
